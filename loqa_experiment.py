"""
Head-to-head experiments: level-k Q-learners (ARAMARL) vs a LOQA adversary
on the iterated Chicken game. Protocol mirrors Experiment2-C.ipynb (the
WoLF-PHC comparison of the previous revision round): stateless repeated
game, 20 steps/episode x 1000 episodes, 10 seeds, moving-average-100 plots.
"""
...
from agent import (IndQLearningAgent, FPLearningAgent, Level2QAgent,
                   Level3QAgentMixDir)
from loqa_agent import LOQAAgent
...
# Chicken payoffs, same convention as Experiment2-C.ipynb:
...
def run_matchup(dm_kind, lr=0.05, tau=1.0, eps=0.05):
    r0ss, r1ss = [], []
    for n in range(N_EXP):
        ...
        dm = make_dm(dm_kind)
        adversary = LOQAAgent(ACTIONS, ACTIONS, learning_rate=lr,
                              epsilon=eps, tau=tau)
        s = 0
        r0s, r1s = [], []
...
if __name__ == '__main__':
    ...
    for kind in ['L0', 'L1', 'L2', 'L3mix']:
        ...
        plot_matchup(r0ss, r1ss, f'img/{kind}vsLOQA_C.png')
        tail = 5000
        mA = r0ss[:, -tail:].mean()
        mB = r1ss[:, -tail:].mean()
        summary.append((kind, mA, mB))
        print(f'{kind:6s} vs LOQA | mean last-{tail} reward: '
              f'DM (A) = {mA:+.3f}   LOQA (B) = {mB:+.3f}')

    print('\nSummary (Chicken reference points: cave = -2, bully = +1, '
          'crash = -4, coordinate = 0):')
